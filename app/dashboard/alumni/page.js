...
export default function AlumniDashboard() {
  ...
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged(async (user) => {
      ...
      if (data.role !== 'alumni' || data.status !== 'active') {
        // Kalau mau cek status juga, misal hanya role alumni dengan status aktif boleh akses
        router.push('/login')
        return
      }
  ...
  return (
    <div className="p-8">
      <h1 className="text-3xl font-bold mb-4">Dashboard Alumni</h1>
